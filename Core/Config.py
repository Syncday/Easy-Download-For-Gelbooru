...
class Config:
    ...
    def read_config(self,config_file_path=None)->bool:
        if config_file_path is None or len(config_file_path)==0:
            config_file_path = path.dirname(__file__)+"/config.json"
        try:
            config_content = ''
            ...
            print("读取配置文件出错[%s]"%e)
            if path.exists(config_file_path) is False:
                self.create_config()
            return False

    def create_config(self):
        self.set('save_path',path.dirname(__file__))
        ...
        if path.exists(config_file_path):
            ...
            try:
                with open(config_file_path, 'w+', -1, 'utf-8') as file:
                    ...
                    file.write("#file_name_format的例子（注意大小写）：[Artist]-Id-Tag\n")
                    file.write("#下载的图片则会是：[作者名]-12322-tag1,tag2\n")
